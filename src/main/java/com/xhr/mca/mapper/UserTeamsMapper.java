...
public interface UserTeamsMapper extends CoreMapper<UserTeams> {
	...
	@Select("select u.phone,u.email,u.img_url as imgUrl,u.nickname,u.level,u.node_level as nodeLevel from `user` u left join user_auth ua on u.id = ua.user_id where u.id in (${ids}) and (ua.`status` = 1) limit #{page},#{rows}")
	...
	@Select("select u.phone,u.email,u.img_url as imgUrl,u.nickname,u.level,u.node_level as nodeLevel from `user` u where u.id in (${ids}) limit #{page},#{rows}")
	public List<User> selectTeamAllUsers(@Param(value = "ids") String ids, @Param(value = "page") int page,
			@Param(value = "rows") int rows) throws WebAppException;

	@Select("select u.phone,u.email,u.img_url as imgUrl,u.nickname,u.level,u.node_level as nodeLevel from `user` u left join user_auth ua on u.id = ua.user_id where u.id in (${ids}) and (ua.`status` != 1 or ua.user_id is null) limit #{page},#{rows}")
