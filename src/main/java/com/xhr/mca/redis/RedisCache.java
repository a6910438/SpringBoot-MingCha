...
public class RedisCache<K, V> {
     ...
     */
    public boolean hmset(K key, Map<String, V> map) {
        try {
     ...
     * @return true成功 false失败
     */
    public boolean hmset(K key, Map<String, V> map, long time) {
        try {
            redisTemplate.opsForHash().putAll(key, map);
            if (time > 0) {
                expire(key, time);
     ...
     * @return 成功个数
     */
    public long sSetAndTime(K key, long time, Object... values) {
        try {
     ...
     * @return
     */
    public List<Object> lGet(K key, long start, long end) {
        try {
